...
function y = rbf(x, centers, type)
    % x: input (n x N)
    % centers: RBF centers (n x Ncenters)
    % type: string, e.g., 'thinplate'
    %
    % Returns: RBF evaluated at all inputs/centers (Ncenters x N)
    [n, N] = size(x);
    [n2, Ncent] = size(centers);
    assert(n == n2, 'Dimension mismatch');
    y = zeros(Ncent, N);
    for i = 1:Ncent
        r = sqrt(sum((x - centers(:,i)).^2, 1));
        if strcmp(type, 'thinplate')
            y(i,:) = r.^2 .* log(r + 1e-8);  % thin-plate RBF
        else
